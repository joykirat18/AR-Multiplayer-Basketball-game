...
public class Shoot1 : MonoBehaviourPun
{
	...
	void FixedUpdate()
	{
		...
		if (arrow.transform.position.x < 68f && right)
		{
			arrow.transform.position += new Vector3(arrowSpeed, 0, 0);
		}
		if (arrow.transform.position.x >= 68f)
		{
			right = false;
		}
		if (right == false)
		...
		if (arrow.transform.position.x <= 50f)
		{
			right = true;
		}
	...
	public void ShootBall()
	{
		if (!thrown)
		{
			...
			Debug.Log(ballClone);
			throwSpeed.y = throwSpeed.y + arrow.transform.position.x - 59;
			throwSpeed.z = throwSpeed.z + arrow.transform.position.x - 59;
